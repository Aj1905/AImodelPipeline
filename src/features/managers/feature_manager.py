...
from ...core.interfaces.feature_interface import IFeatureManager
...
class FeatureManager(IFeatureManager):
    ...
    def __str__(self) -> str:
        ...
        if enabled_features:
            result += f"  有効特徴量: {enabled_features[:5]}"
            if len(enabled_features) > 5:
                result += f" ... (他{len(enabled_features) - 5}個)"
            result += "\n"

        if disabled_features:
            result += f"  無効特徴量: {disabled_features[:5]}"
            if len(disabled_features) > 5:
                result += f" ... (他{len(disabled_features) - 5}個)"
            result += "\n"
    ...
    def _validate_dataframe_length(self, df: pl.DataFrame) -> None:
        ...
        Raises:
            ValueError: 特徴量の長さが他の特徴量と一致しない場合
        """
        if self._features is not None and not self._features.is_empty():
            if df.height != self._features.height:
                raise ValueError(
                    f"特徴量の長さが一致しません。既存: {self._features.height}, 新規: {df.height}"
                )
